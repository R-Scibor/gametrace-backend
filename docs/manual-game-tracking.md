...
- **Rate limits** — shipped. `POST /games/match` is capped at 20/hour and `POST /games` at 60/hour, per user, Redis-backed (same mechanism as `/voice/transcribe`'s hourly window). IGDB throttles per Client ID, so the whole deployment plus the enrichment worker share one budget; the caps stop a looping client from draining it.
