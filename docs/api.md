...
| `GET` | `/api/v1/stats/dashboard` | Polling tile endpoint — fixed `total_seconds_7d` + `total_seconds_30d`, the active `ONGOING` session brief, and `pending_errors`. No per-game breakdown. Designed for 30s polling on the Dashboard tab. |
