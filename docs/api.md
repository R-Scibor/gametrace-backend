...
| `PATCH` | `/api/v1/sessions/{id}` | Edit `end_time` on a `COMPLETED` or `ERROR` session. To delete a session (`ERROR` or `COMPLETED`), use `DELETE /api/v1/sessions/{id}`. Editing `ONGOING` is forbidden — those are bot-managed. |
