...
| `POST` | `/api/v1/profile/me/deletion` | Schedule the current account for deletion. No request body. Stamps `deletion_requested_at = now`, `purge_at = now + ACCOUNT_DELETION_GRACE_DAYS`, revokes every `UserAuthToken` and `UserDevice` for the user, transitions any `ONGOING` session to `ERROR`, and best-effort flushes any pending login-link code. `COMPLETED`/`ERROR` sessions are untouched. Idempotent — calling again while already scheduled returns the existing `deletion_requested_at`/`purge_at` unchanged, no re-work. `403` if the caller is an admin (see below). Returns `202` `DeletionStatusResponse`: `{deletion_requested_at, purge_at, days_left}`. The account remains readable/usable during the grace period only via this endpoint and `POST /auth/logout`; every other authed route 403s once `purge_at` is set. A later scheduled task permanently purges the row once `purge_at` passes. |
