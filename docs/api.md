...
| `PATCH` | `/api/v1/admin/reports/{id}` | Triage a single report. Body: `{"status": "open" \| "triaged" \| "closed"}` — any status may transition to any other, including reopening a `closed` or `triaged` report. Any other value is rejected with `422`. Returns `200` with the updated report item (same shape as a list item). `404` if `id` does not exist. Audit action: `report_triage`, resource `report:{id}`, with `before`/`after` status. |
